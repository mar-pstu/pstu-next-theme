    ...
    if ( $sticky_category_id = get_translate_id( get_theme_mod( 'sticky_category_id', false ), 'category' ) ) {
      ...
      if ( ( $sticky_category ) && ( ! empty( $sticky_category ) ) && ( ! is_wp_error( $sticky_category ) ) ) {
        $sticky_entry = array(
          ...
          'thumbnail'  => get_term_meta( $sticky_category->term_id, 'pstu_thumbnail', false ),
        );
      }
...
if ( ( $sticky_entry ) && ( ! empty( $sticky_entry ) ) ) {
  ...
  if ( $sticky_entry[ 'thumbnail' ] ) {
    echo "        <div class=\"col-xs-12 col-sm-4 col-md-3 col-lg-2 text-center\">\r\n";
    echo "          <a href=\"" . $sticky_entry[ 'link' ] . "\">\r\n";
    echo "            <img class=\"wp-post-image lazy\" src=\"#\" data-src=\"" . $sticky_entry[ 'thumbnail' ] . "\" alt=\"" . esc_attr( $sticky_entry[ 'title' ] ) . "\">\r\n";
    echo "          </a>\r\n";
    echo "        </div>\r\n";
  }
  echo "        <div class=\"col-xs-12 col-sm col-md col-lg\">\r\n";
  echo "          <h3 class=\"title\"><a href=\"" . $sticky_entry[ 'link' ] . "\" style=\"color: " . $sticky_text_color . ";\" title=\"" . sprintf( "%s - %s", __( 'Подробней', 'pstu-next-theme' ), esc_attr( $sticky_entry[ 'title' ] ) ) . "\">" . $sticky_entry[ 'title' ] . "</a></h3>\r\n";
  if ( $sticky_entry[ 'excerpt' ] ) echo "          <div class=\"excerpt\" style=\"color: " . $sticky_text_color . ";\">" . $sticky_entry[ 'excerpt' ] . "</div>\r\n";
  echo "        </div>\r\n"; // .col-
  echo "        <div class=\"col-xs-12 col-sm-12 col-md-3 col-lg-2 text-right\">\r\n";
  echo "          <a class=\"permalink btn btn-success btn-md\" href=\"" . $sticky_entry[ 'link' ] . "\" title=\"" . sprintf( "%s - %s", __( 'Подробней', 'pstu-next-theme' ), esc_attr( $sticky_entry[ 'title' ] ) ) . "\">" . __( 'Подробней', 'pstu-next-theme' ) . "</a>\r\n";
  echo "        </div>\r\n"; // .col-
  echo "      </div>\r\n"; // .row
  echo "    </div>\r\n"; // .container
  echo "  </div>\r\n"; // .sticky__entry
