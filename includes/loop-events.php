...
if ( have_posts() ) {
	...
	while ( have_posts() ) {
		...
		$events_entry_date = pstu_next_get_event_date( get_the_title( get_the_ID() ) );
		$events_entry_classes = array( 'media', 'clearfix' );
		if ( $events_entry_date[ 'date' ] < time() ) $events_entry_classes = array_merge( $events_entry_classes, array( 'past-event' ) );

		echo "<div class=\"" . implode( ' ', get_post_class( $events_entry_classes, get_the_ID() ) ) . "\">\r\n";
		echo "  <div class=\"media-left pull-left\">\r\n";
		echo pstu_next_get_date_box( $events_entry_date );
		echo "  </div>\r\n"; // .media-left
