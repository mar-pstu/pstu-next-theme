...
if ( ( $prev_post ) && ( ! is_wp_error( $prev_post ) ) && ( ! empty( $prev_post ) ) ) {
	$result[] = "<a class=\"next pager__item item\" href=\"" . get_permalink( $next_post->ID ) . "\" title=\"" . printf( "%s - %s", __( 'Подробней', 'pstu-next-theme' ), esc_attr( $prev_post->post_title ) ) . "\">";
...
if ( ( $next_post ) && ( ! is_wp_error( $next_post ) ) && ( ! empty( $next_post ) ) ) {
	$result[] = "<a class=\"previous pager__item item\" href=\"" . get_permalink( $next_post->ID ) . "\" title=\"" . printf( "%s - %s", __( 'Подробней', 'pstu-next-theme' ), esc_attr( $next_post->post_title ) ) . "\">";
